...
int PropagateLCIOToNewRef(IMPL::TrackStateImpl& ts, double xref, double yref, double zref) {
  ...
  const double d0Prime = d0 + deltaX * sinPhi0 - deltaY * cosPhi0 +
                         ((deltaX * cosPhi0 + deltaY * sinPhi0) * tan((phi0Prime - phi0) / 2.0));
  ...
  const double cosDeltaPhi = 1.0 + (omega * omega / (2.0 * (1.0 - omega * d0Prime))) *
                                       (d0Prime * d0Prime - (deltaX + d0 * sinPhi0) * (deltaX + d0 * sinPhi0) -
                                        (deltaY - d0 * cosPhi0) * (deltaY - d0 * cosPhi0));
...
// Propagate track to a new reference point taken as its crossing point with a cylinder of infinite length centered at
// x0,y0, parallel to the z axis. For direction== 0  the closest crossing point will be taken For direction== 1  the
// first crossing traversing in positive s will be taken For direction==-1  the first crossing traversing in negative s
// will be taken
...
int PropagateLCIOToCylinder(IMPL::TrackStateImpl& ts, float r0, float x0, float y0, int direction, double epsilon) {
  ...
  //    std::cout << "PropagateLCIOToCylinder: r = " << r0 << " x0:y0 = " << x0 << " : " << y0 << " direction = " <<
  //    direction << std::endl ;
...
// Propagate track to a new reference point taken as its crossing point with a plane parallel to the z axis, containing
// points x1,x2 and y1,y2. Tolerance for intersection determined by epsilon. For direction ==  0  the closest crossing
// point will be taken For direction ==  1  the first crossing traversing in positive s will be taken For direction ==
// -1  the first crossing traversing in negative s will be taken
int PropagateLCIOToPlaneParralelToZ(IMPL::TrackStateImpl& ts, float x1, float y1, float x2, float y2, int direction,
                                    double epsilon) {
  ...
  //    std::cout << "PropagateLCIOToPlaneParralelToZ: 1st solution u = " << u1 << " : " << u1 * dx << " " << u1 * dy <<
  //    std::endl ; std::cout << "PropagateLCIOToPlaneParralelToZ: 2nd solution u = " << u2 << " : " << u2 * dx << " "
  //    << u2 * dy << std::endl ; std::cout << "PropagateLCIOToPlaneParralelToZ: 1st solution x:y = " << x_ins1 << " : "
  //    << y_ins1 << std::endl ; std::cout << "PropagateLCIOToPlaneParralelToZ: 2nd solution x:y = " << x_ins2 << " : "
  //    << y_ins2 << std::endl ;
  ...
  if (direction == 0) { // take closest intersection
    ...
    if (s_1 < 0.0)
      s_1 += 2.0 * M_PI * fabs(rho);
    if (s_2 < 0.0)
      s_2 += 2.0 * M_PI * fabs(rho);
