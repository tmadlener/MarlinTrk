...
DiagnosticsController::DiagnosticsController() {
  ...
  _initialised = false;
  _recording_on = false; // recording is set to off by default so that processors do not have to perform any action if
                         // they are not interested in diagnostics, i.e. no need to call init
  _current_track = 0;
  _currentMCP = 0;
...
void DiagnosticsController::new_track(MarlinKalTestTrack* trk) {
  ...
  if (_initialised == false) {
    streamlog_out(ERROR)
        << "DiagnosticsController::new_track: Diagnostics not initialised call init(std::string root_file_name, "
           "std::string root_tree_name, bool recording_off) first : exit(1) called from file "
...
void DiagnosticsController::set_intial_track_parameters(double d0, double phi0, double omega, double z0, double tanL,
  ...
  if (_initialised == false) {
    streamlog_out(ERROR)
        << "DiagnosticsController::set_intial_track_parameters: Diagnostics not initialised call init(std::string "
           "root_file_name, std::string root_tree_name, bool recording_off) first : exit(1) called from file "
        << __FILE__ << " line " << __LINE__ << std::endl;
  ...
  streamlog_out(DEBUG3) << " $#$#$# Initial Track Parameters: "
                        << "d0 = " << d0 << " "
                        << "[+/-" << sqrt(_track_record->cov_seed_d0d0) << "] "
                        << "phi0 = " << phi0 << " "
                        << "[+/-" << sqrt(_track_record->cov_seed_phi0phi0) << "] "
                        << "omega = " << omega << " "
                        << "[+/-" << sqrt(_track_record->cov_seed_kappakappa) << "] "
                        << "z0 = " << z0 << " "
                        << "[+/-" << sqrt(_track_record->cov_seed_z0z0) << "] "
                        << "tanL = " << tanL << " "
                        << "[+/-" << sqrt(_track_record->cov_seed_tanLtanL) << "] "
                        << "ref = " << pivot_x << " " << pivot_y << " " << pivot_z << " " << std::endl;
}
...
void DiagnosticsController::record_rejected_site(ILDVTrackHit* hit, TKalTrackSite* site) {
  ...
  if (_initialised == false) {
    streamlog_out(ERROR)
        << "DiagnosticsController::record_rejected_site: Diagnostics not initialised call init(std::string "
           "root_file_name, std::string root_tree_name, bool recording_off) first : exit(1) called from file "
        << __FILE__ << " line " << __LINE__ << std::endl;
  ...
  streamlog_out(DEBUG2) << "record_rejected_site _track_record->nsites = " << _track_record->nsites
                        << " MCParticle of simhit = " << mcp << " pdg " << mcp->getPDG()
                        << " energy = " << mcp->getEnergy() << " id = " << mcp->id() << std::endl;
}
...
void DiagnosticsController::record_site(ILDVTrackHit* hit, TKalTrackSite* site) {
  ...
  if (_initialised == false) {
    streamlog_out(ERROR)
        << "DiagnosticsController::record_site: Diagnostics not initialised call init(std::string root_file_name, "
           "std::string root_tree_name, bool recording_off) first : exit(1) called from file "
        << __FILE__ << " line " << __LINE__ << std::endl;
  ...
  if (!ext) {
    ...
    streamlog_out(ERROR) << "MCTruth4HitExt not attached to TrackerHit use processor SetTrackerHitExtensions to set "
                            "them: exit(1) called from file "
                         << __FILE__ << " line " << __LINE__ << std::endl;
  ...
  if (!simhit) {
    ...
    streamlog_out(ERROR) << "SimTrackerHit not attached to TrackerHit using MCTruth4HitExt use processor "
                            "SetTrackerHitExtensions to set them: exit(1) called from file "
                         << __FILE__ << " line " << __LINE__ << std::endl;
  ...
  if (_track_record->nsites >= 0) {
    ...
    if (_mcpInfoStored == false) {
      ...
      streamlog_out(DEBUG2) << "DiagnosticsController::record_site store MCParticle parameters for " << mcp << " pdg "
                            << mcp->getPDG() << " energy = " << mcp->getEnergy() << " id = " << mcp->id() << std::endl;
      ...
      double pt = sqrt(_track_record->px_mcp * _track_record->px_mcp + _track_record->py_mcp * _track_record->py_mcp);
      ...
      streamlog_out(WARNING) << "DiagnosticsController::record_site: Track skipped. Not storing tracks formed from "
                                "more than one MCParticle "
                             << std::endl;
      return;
    }
    ...
    if (fabs(sim_mom[0]) < 1.e-09 && fabs(sim_mom[1]) < 1.e-09 && fabs(sim_mom[2]) < 1.e-09) {
      ...
      streamlog_out(ERROR) << "Momentum not set in SimTrackerHit exit(1) called from file " << __FILE__ << " line "
                           << __LINE__ << std::endl;
    ...
    // here perhaps we should move the helix to the hit to calculate the deltas or though this could still be done in
    // the analysis code, as both sim and rec hit positions are stored ?
    //      helixMC.moveRefPoint(0.0, 0.0, 0.0);
    ...
    streamlog_out(DEBUG2) << " $#$#$# SimHit Track Parameters: "
                          ...
                          << " mcp ID = " << simhit->getMCParticle()->id() << std::endl;
    ...
    TKalTrackState& trkState_predicted =
        (TKalTrackState&)site->GetState(TVKalSite::kPredicted); // this segfaults if no hits are present
    ...
    TKalTrackState& trkState_filtered =
        (TKalTrackState&)site->GetState(TVKalSite::kFiltered); // this segfaults if no hits are present
  ...
  if (_track_record->nsites > MAX_SITES) {
    _skip_track = true;
    this->clear_track_record();
    streamlog_out(DEBUG4) << " DiagnosticsController::end_track: Number of site too large. Track Skipped.    nsites = "
                          << _track_record->nsites << " : maximum number of site allowed = " << MAX_SITES << std::endl;
    return;
  }
...
void DiagnosticsController::end_track() {
  ...
  if (_initialised == false) {
    streamlog_out(ERROR)
        << "DiagnosticsController::end_track: Diagnostics not initialised call init(std::string root_file_name, "
           "std::string root_tree_name, bool recording_off) first : exit(1) called from file "
        << __FILE__ << " line " << __LINE__ << std::endl;
  ...
  if (_skip_track) { // just clear the track buffers and return.
    ...
    if (nsites > MAX_SITES) {
      ...
      streamlog_out(DEBUG4)
          << " DiagnosticsController::end_track: Number of site too large. Track Skipped.    nsites = " << nsites
          << " : maximum number of site allowed = " << MAX_SITES << std::endl;
      return;
    }
    ...
    if (_current_track->_smoothed) {
      ...
      for (Int_t isite = 1; isite < nsites; isite++) {
        ...
        if (_track_record->rejected[isite] == 0 && encoder[lcio::LCTrackerCellID::subdet()] != 0) {
          ...
          TKalTrackState& trkState_smoothed =
              (TKalTrackState&)site->GetState(TVKalSite::kSmoothed); // this segfaults if no hits are present
          ...
          // move track state to the sim hit for comparison
          const TVector3 tpoint(_track_record->ref_point_x[isite], _track_record->ref_point_y[isite],
                                _track_record->ref_point_z[isite]);
          ...
          helix_smoothed.MoveTo(tpoint, dPhi, &DF,
                                &c0_smoothed); // move helix to desired point, and get propagator matrix
...
void DiagnosticsController::end() {
  ...
  if (_initialised == false) {
    streamlog_out(ERROR)
        << "DiagnosticsController::end: Diagnostics not initialised call init(std::string root_file_name, std::string "
           "root_tree_name, bool recording_off) first : exit(1) called from file "
        << __FILE__ << " line " << __LINE__ << std::endl;
  ...
  streamlog_out(DEBUG4) << " DiagnosticsController::end() called \n"
                        ...
                        << "\t number of tracks skipped = " << _ntracks_skipped << std::endl;
