...
MarlinDDKalTest::MarlinDDKalTest() : _ipLayer(NULL) {
...
void MarlinDDKalTest::init() {
  ...
  streamlog_out(DEBUG5) << " -------------------------------------------------------------------------------- "
                        << std::endl;
  streamlog_out(DEBUG5) << "  MarlinDDKalTest::init() called with the following options :                     "
                        << std::endl;
  ...
  for (std::vector<dd4hep::DetElement>::iterator it = detectors.begin(); it != detectors.end(); ++it) {
    ...
    if (streamlog_level(DEBUG5)) { // dump surfaces to text file
      ...
      std::ofstream file;
      std::stringstream s;
      s << "DDKalTest_" << det.name() << "_surfaces.txt";
      ...
      for (std::map<double, DDVMeasLayer*>::iterator itm = smap.begin(); itm != smap.end(); ++itm) {
        ...
        file << " " << std::scientific << std::setw(10) << itm->first << "\t" << bf.valueString()
             << *itm->second->surface() << "\n";
      }
      file.close();
    }
...
MarlinTrk::IMarlinTrack* MarlinDDKalTest::createTrack() {
  ...
  if (!is_initialised) {
    ...
    errorMsg << "MarlinDDKalTest::createTrack: Fitter not initialised. MarlinDDKalTest::init() must be called before "
                "MarlinDDKalTest::createTrack()"
             << std::endl;
...
void MarlinDDKalTest::getSensitiveMeasurementModulesForLayer(int layerID,
                                                             std::vector<const DDVMeasLayer*>& measmodules) const {

  if (!measmodules.empty()) {

    std::stringstream errorMsg;
    errorMsg << "MarlinDDKalTest::getSensitiveMeasurementModulesForLayer vector passed as second argument is not empty "
             << std::endl;
    throw MarlinTrk::Exception(errorMsg.str());
  }

  streamlog_out(DEBUG0) << "MarlinDDKalTest::getSensitiveMeasurementModulesForLayer: layerID = " << layerID
                        << std::endl;
  ...
  std::pair<std::multimap<Int_t, const DDVMeasLayer*>::const_iterator,
            std::multimap<Int_t, const DDVMeasLayer*>::const_iterator>
      ii;
...
void MarlinDDKalTest::getSensitiveMeasurementModules(int moduleID,
                                                     std::vector<const DDVMeasLayer*>& measmodules) const {

  if (!measmodules.empty()) {

    std::stringstream errorMsg;
    errorMsg << "MarlinDDKalTest::getSensitiveMeasurementLayer vector passed as second argument is not empty "
             << std::endl;
    throw MarlinTrk::Exception(errorMsg.str());
  }

  std::pair<std::multimap<int, const DDVMeasLayer*>::const_iterator,
            std::multimap<Int_t, const DDVMeasLayer*>::const_iterator>
      ii;
...
const DDVMeasLayer* MarlinDDKalTest::getLastMeasLayer(THelicalTrack const& hel, TVector3 const& point) const {
  ...
  //  streamlog_out( DEBUG4 ) << "  MarlinDDKalTest - getLastMeasLayer deflection to point = " << deflection_to_point <<
  //  " kappa = " << helix.GetKappa()  << "  mode = " << mode << std::endl ; streamlog_out( DEBUG4 ) << " Point to move
  //  to:" << std::endl; point.Print();
  ...
  if (meas_modules.size() == 0) { // no measurement layers found
    ...
    for (unsigned int i = 0; i < meas_modules.size(); ++i) {
      ...
      if (!(surf = dynamic_cast<const TVSurface*>(meas_modules[i]))) {
        std::stringstream errorMsg;
        errorMsg << "MarlinDDKalTest::findMeasLayer dynamic_cast failed for surface type: moduleID = " << detElementID
                 << std::endl;
        throw MarlinTrk::Exception(errorMsg.str());
      }
      ...
      if ((!surf_found) && hit_on_surface) {
        ...
        std::stringstream errorMsg;
        errorMsg << "MarlinDDKalTest::findMeasLayer point found to be on two surfaces: moduleID = " << detElementID
                 << std::endl;
        throw MarlinTrk::Exception(errorMsg.str());
      }
    }
    if (!surf_found) { // print out debug info
      streamlog_out(DEBUG1)
          << "MarlinDDKalTest::findMeasLayer point not found to be on any surface matching moduleID = " << detElementID
