...
MarlinAidaTTTrack::~MarlinAidaTTTrack() { delete _fitTrajectory; }
...
int MarlinAidaTTTrack::initialise(bool /*fitDirection*/) {
  ...
  if (nHits < 3) {
    ...
    streamlog_out(ERROR) << "<<<<<< MarlinAidaTTTrack::initialise: Shortage of Hits! nhits = " << _lcioHits.size()
                         << " >>>>>>>" << std::endl;
    return error;
  }
...
int MarlinAidaTTTrack::fit(double) {
  ...
  int pointLabel = 0;
  for (std::vector<std::pair<double, const aidaTT::ISurface*>>::const_iterator it = _intersections->begin();
       it != _intersections->end(); ++it) {
    ...
    streamlog_out(DEBUG7) << "MarlinAidaTTTrack::fit() - intersection - current pointLabel : " << pointLabel
                          << ":  at s = " << it->first << " surface id : " << cellIDString(surf->id()) << std::endl
                          << *surf << std::endl;
    ...
    if (hit != 0) { //-------- we have to add a measurement
      ...
      streamlog_out(DEBUG) << "MarlinAidaTTTrack::fit()  addMeasurement called for pointLabel : " << pointLabel
                           << std::endl;
      ...
      if (_aidaTT->_useQMS) {
        ...
        if (!(surf->innerMaterial().density() < 1e-6 && surf->outerMaterial().density() < 1e-6)) {

          _fitTrajectory->addScatterer(*surf);
          _indexMap[surf->id()] = ++pointLabel; // label 0 is for the IP point

          streamlog_out(DEBUG) << "MarlinAidaTTTrack::fit()  addScatterer called for pointLabel : " << pointLabel
                               << std::endl;
        }
      }
    }
...
aidaTT::trackParameters MarlinAidaTTTrack::createPreFit(aidaTT::trackParameters& tp) {
  ...
  streamlog_out(DEBUG1) << " MarlinAidaTTTrack::createPreFit() :  will use every " << step << "-th hit for prefit ! "
                        << std::endl;
  ...
  for (unsigned i = 0; i < nHits; i += step) {
    ...
    if (it == _aidaTT->_surfMap.end()) {
      ...
      streamlog_out(DEBUG1) << " MarlinAidaTTTrack::createPreFit() : no surface found for id : " << cellIDString(hitid)
                            << std::endl;
      continue;
    }
    ...
    streamlog_out(DEBUG1) << " MarlinAidaTTTrack::createPreFit() : adding hit for " << cellIDString(hitid) << " at "
  ...
  if (success) {
    ...
    streamlog_out(DEBUG4) << " MarlinAidaTTTrack::createPreFit() : prefit tp: " << newTP << std::endl;

    return newTP;

  } else {

    streamlog_out(WARNING) << " MarlinAidaTTTrack::createPreFit() : prefit failed for tp: " << tp << std::endl;

    return tp;
  }
}

void MarlinAidaTTTrack::getHitInfo(const EVENT::TrackerHit* hit, double* hitpos, std::vector<double>& precision,
                                   const aidaTT::ISurface* surf) {

  // get the hit position in dd4hep/aidaTT units
  for (unsigned int i = 0; i < 3; ++i)
    hitpos[i] = hit->getPosition()[i] * dd4hep::mm;
...
int MarlinAidaTTTrack::getTrackState(EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts, double& chi2, int& ndf) {
  ...
  if (it == _indexMap.end()) {
    ...
    streamlog_out(DEBUG2) << " MarlinAidaTTTrack::getTrackState(): "
                          << " no surface intersection for given hit " << cellIDString(trkhit->getCellID0())
                          << std::endl;
    return error;
  }
...
int MarlinAidaTTTrack::getTrackState(const aidaTT::Vector3D& refPoint, int label, IMPL::TrackStateImpl& ts,
                                     double& chi2, int& ndf) {
  ...
  if (result == 0) {
    ...
    streamlog_out(DEBUG2) << " MarlinAidaTTTrack::getTrackState(): "
                          << " no result at label " << label << " close to " << refPoint << std::endl;
    return error;
  }
...
int MarlinAidaTTTrack::extrapolate(const Vector3D& point, EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts,
                                   double& chi2, int& ndf) {

  return propagate(point, trkhit, ts, chi2, ndf);
}

int MarlinAidaTTTrack::extrapolateToLayer(int layerID, IMPL::TrackStateImpl& ts, double& chi2, int& ndf,
                                          int& detElementID, int mode) {

  return propagateToLayer(layerID, ts, chi2, ndf, detElementID, mode);
}

int MarlinAidaTTTrack::extrapolateToLayer(int layerID, EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts,
                                          double& chi2, int& ndf, int& detElementID, int mode) {

  return propagateToLayer(layerID, trkhit, ts, chi2, ndf, detElementID, mode);
}

int MarlinAidaTTTrack::extrapolateToDetElement(int detElementID, IMPL::TrackStateImpl& ts, double& chi2, int& ndf,
                                               int mode) {

  return propagateToDetElement(detElementID, ts, chi2, ndf, mode);
}

int MarlinAidaTTTrack::extrapolateToDetElement(int detElementID, EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts,
...
int MarlinAidaTTTrack::propagate(const Vector3D& point, IMPL::TrackStateImpl& ts, double& chi2, int& ndf) {
  ...
  if (point[0] == 0.0 && point[1] == 0.0 && point[2] == 0.0) {
    ...
    return getTrackState(aidaTT::Vector3D(point[0], point[1], point[2]), label, ts, chi2, ndf);
  }
}

int MarlinAidaTTTrack::propagate(const Vector3D& point, EVENT::TrackerHit*, IMPL::TrackStateImpl& ts, double& chi2,
                                 int& ndf) {

  return this->propagate(point, ts, chi2, ndf);
}

int MarlinAidaTTTrack::propagateToLayer(int layerID, IMPL::TrackStateImpl& ts, double& chi2, int& ndf,
                                        int& detElementID, int) {
  ...
  if (theID == -1) {
    streamlog_out(ERROR) << "MarlinAidaTTTrack::propagate(): no intersection "
                         << " found for layerID " << cellIDString(layerID) << std::endl;
...
int MarlinAidaTTTrack::propagateToLayer(int layerID, EVENT::TrackerHit*, IMPL::TrackStateImpl& ts, double& chi2,
                                        int& ndf, int& detElementID, int) {
...
int MarlinAidaTTTrack::propagateToDetElement(int detElementID, IMPL::TrackStateImpl& ts, double& chi2, int& ndf, int) {
  ...
  if (it == _indexMap.end()) {
    ...
    streamlog_out(DEBUG2) << " MarlinAidaTTTrack::propagateToDetElement(): "
                          << " no surface intersection for given DetElementID " << cellIDString(detElementID)
                          << std::endl;
    return error;
  }
...
int MarlinAidaTTTrack::propagateToDetElement(int detElementID, EVENT::TrackerHit*, IMPL::TrackStateImpl& ts,
                                             double& chi2, int& ndf, int) {
...
int MarlinAidaTTTrack::intersectionWithDetElement(int detElementID, Vector3D& point, int) {
  ...
  //    if( it == _aidaTT->_surfMap.end() ){
  //      streamlog_out( DEBUG2 )  << " MarlinAidaTTTrack::intersectionWithDetElement() - no surface found for " <<
  //      cellIDString( detElementID ) << std::endl ; return error ;
  //    }
  //    const aidaTT::ISurface* surf = it->second ;
  //    double s ; aidaTT::Vector2D uv ; aidaTT::Vector3D position ;
...
std::string MarlinAidaTTTrack::toString() { return std::string(" AidaTTTrack - to String ... "); }
