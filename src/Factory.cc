...
IMarlinTrkSystem* Factory::createMarlinTrkSystem(const std::string& systemType, const gear::GearMgr*,
  ...
  streamlog_out(MESSAGE) << " Factory::createMarlinTrkSystem:  creating IMarlinTrkSystem of type \"" << systemType
                         << "\"" << std::endl;
...
IMarlinTrkSystem* Factory::getCurrentMarlinTrkSystem() {
  ...
  if (current == 0) {
    ...
    std::stringstream log;
    log << " Factory::getCurrentMarlinTrkSystem called without a preceeding call to createMarlinTrkSystem() ot "
           "getMarlinTrkSystem() ";
  ...
  streamlog_out(DEBUG4)
      << " Factory::getCurrentMarlinTrkSystem() called - return allready initialized IMarlinTrkSystem " << std::endl;
