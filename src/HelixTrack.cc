...
HelixTrack::HelixTrack(const double* x1, const double* x2, const double* x3, double Bz, bool direction) {
  ...
  streamlog_out(DEBUG2) << "HelixTrack::HelixTrack Create from hits: \n "
                        ...
                        << "\n "
                        << "Bz = " << Bz << " direction = " << direction << std::endl;
...
double HelixTrack::moveRefPoint(double x, double y, double z) {
  ...
  const double d0Prime = _d0 + deltaX * sinPhi0 - deltaY * cosPhi0 +
                         ((deltaX * cosPhi0 + deltaY * sinPhi0) * tan((phi0Prime - _phi0) / 2.0));
  ...
  const double cosDeltaPhi = 1.0 + (_omega * _omega / (2.0 * (1.0 - _omega * d0Prime))) *
                                       (d0Prime * d0Prime - (deltaX + _d0 * sinPhi0) * (deltaX + _d0 * sinPhi0) -
                                        (deltaY - _d0 * cosPhi0) * (deltaY - _d0 * cosPhi0));
