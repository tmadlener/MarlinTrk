...
public:
  ...
  virtual Bool_t IsAccepted(const TKalTrackSite& site) {
    ...
    streamlog_out(DEBUG1) << " KalTrackFilter::IsAccepted called  !  deltaChi2 = " << std::scientific << deltaChi2
                          << " _maxDeltaChi2 = " << _maxDeltaChi2 << std::endl;
...
MarlinDDKalTestTrack::MarlinDDKalTestTrack(MarlinDDKalTest* ktest) : _ktest(ktest) {
...
int MarlinDDKalTestTrack::addHit(EVENT::TrackerHit* trkhit, const DDVMeasLayer* ml) {
  ...
  streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::addHit: trkhit = " << trkhit->id() << " addr: " << trkhit
                        << " ml = " << ml << std::endl;
...
int MarlinDDKalTestTrack::addHit(EVENT::TrackerHit* trkhit, DDVTrackHit* kalhit, const DDVMeasLayer* ml) {
  ...
  if (kalhit && ml) {
    ...
    _lcio_hits_to_kaltest_hits[trkhit] =
        kalhit; // add hit to map relating lcio and kaltest hits
  ...
  streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::addHit: hit added "
                        << "number of hits for track = " << _kalhits->GetEntries() << std::endl;
...
int MarlinDDKalTestTrack::initialise(bool fitDirection) {
  ...
  // }
    ...
    streamlog_out(ERROR) << "<<<<<<<<< MarlinDDKalTestTrack::initialise: dynamic_cast failed for hit type >>>>>>>"
                         << std::endl;
    return error;
  }
  ...
  if (h1.GetDimension() == 1 || h2.GetDimension() == 1 || h3.GetDimension() == 1) {
    ...
    throw MarlinTrk::Exception("Track fit cannot be initialised from 1 Dimensional hits. Use method "
  ...
  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::initialise Create initial helix from hits: \n "
                        ...
                        << "\n "
                        << "Bz = " << h1.GetBfield() << " direction = " << _fitDirection << std::endl;
  ...
  //      // fg: if the error is too large the initial helix parameters might be changed extremely by the first three
  //      (or so) hits,
  ...
  streamlog_out(DEBUG2) << " track parameters used for init : " << std::scientific << std::setprecision(6) << "\t D0 "
                        << 0.0 << "\t Phi :" << toBaseRange(helstart.GetPhi0() + M_PI / 2.) << "\t Omega "
                        << 1. / helstart.GetRho() << "\t Z0 " << 0.0 << "\t tan(Lambda) " << helstart.GetTanLambda()

                        << "\t pivot : [" << helstart.GetPivot().X() << ", " << helstart.GetPivot().Y() << ", "
                        << helstart.GetPivot().Z() << " - r: "
                        << std::sqrt(helstart.GetPivot().X() * helstart.GetPivot().X() +
                                     helstart.GetPivot().Y() * helstart.GetPivot().Y())
                        << "]" << std::endl;
  ...
  _ktest->_diagnostics.set_intial_track_parameters(d0, phi, omega, z0, tanLambda, helstart.GetPivot().X(),
                                                   helstart.GetPivot().Y(), helstart.GetPivot().Z(), Cov);
...
int MarlinDDKalTestTrack::initialise(const EVENT::TrackState& ts, double /*bfield_z*/, bool fitDirection) {
  ...
  if (_kalhits->GetEntries() == 0) {
    ...
    streamlog_out(ERROR) << "<<<<<< MarlinDDKalTestTrack::Initialise: Number of Hits is Zero. Cannot Initialise >>>>>>>"
                         << std::endl;
    return error;
  }
  ...
  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::initialise using TrackState: track parameters used for init : "
                        << "\t D0 " << ts.getD0() << "\t Phi :" << ts.getPhi() << "\t Omega " << ts.getOmega()
                        << "\t Z0 " << ts.getZ0() << "\t tan(Lambda) " << ts.getTanLambda()

                        << "\t pivot : [" << ts.getReferencePoint()[0] << ", " << ts.getReferencePoint()[1] << ", "
                        << ts.getReferencePoint()[2] << " - r: "
                        << std::sqrt(ts.getReferencePoint()[0] * ts.getReferencePoint()[0] +
  ...
  THelicalTrack helix(-ts.getD0(), toBaseRange(ts.getPhi() - M_PI / 2.), kappa, ts.getZ0(), ts.getTanLambda(),
                      ts.getReferencePoint()[0], ts.getReferencePoint()[1], ts.getReferencePoint()[2], Bz);
  ...
  if ((pDummyHit = dynamic_cast<DDCylinderHit*>(kalhit))) {
    ...
    if (pDummyHit->GetDimension() == 1) {
      ...
      if (surf) {
        ...
        streamlog_out(DEBUG) << "  MarlinDDKalTestTrack::initialise - CalcXingPointWith called for 1d hit ... "
                             << std::endl;

      } else {
        streamlog_out(ERROR) << "<<<<<<<<< MarlinDDKalTestTrack::initialise: dynamic_cast failed for TVSurface  >>>>>>>"
                             << std::endl;
        return error;
      }
    }
  ...
  _ktest->_diagnostics.set_intial_track_parameters(d0, phi, omega, z0, tanLambda, helix.GetPivot().X(),
                                                   helix.GetPivot().Y(), helix.GetPivot().Z(), covK);
...
int MarlinDDKalTestTrack::addAndFit(DDVTrackHit* kalhit, double& chi2increment, TKalTrackSite*& site,
                                    double maxChi2Increment) {

  streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::addAndFit called : maxChi2Increment = " << std::scientific
                        << maxChi2Increment << std::endl;
  ...
  streamlog_out(DEBUG1) << "Kaltrack::addAndFit :  add site to track at index : "
                        << (dynamic_cast<const DDVMeasLayer*>(&(kalhit->GetMeasLayer())))->GetIndex() << " for type "
  ...
  for (unsigned int i = 0; i < (dynamic_cast<const DDVMeasLayer*>(&(kalhit->GetMeasLayer()))->getNCellIDs()); ++i) {
    streamlog_out(DEBUG0) << " : " << dynamic_cast<const DDVMeasLayer*>(&(kalhit->GetMeasLayer()))->getCellIDs()[i];
  }
  ...
  if (!_kaltrack->AddAndFilter(*temp_site)) {
    ...
    streamlog_out(DEBUG2) << "Kaltrack::addAndFit : site discarded! at index : " << ml->GetIndex() << " for type "
                          << ml->GetName() << " chi2increment = " << chi2increment
                          << " maxChi2Increment = " << maxChi2Increment << " x = " << pos.x() << " y = " << pos.y()
                          << " z = " << pos.z() << " with CellIDs: " << std::endl;
    ...
    // and this also works..
    streamlog_out(DEBUG2) << " addAndFit : Site passed Chi2 filter step ? " << filter.passedLastFilterStep()
                          << std::endl;
...
int MarlinDDKalTestTrack::addAndFit(EVENT::TrackerHit* trkhit, double& chi2increment, double maxChi2Increment) {
  ...
  if (!trkhit) {
    streamlog_out(ERROR) << "MarlinDDKalTestTrack::addAndFit( EVENT::TrackerHit* trkhit, double& chi2increment, double "
                            "maxChi2Increment): trkhit == 0"
                         << std::endl;
    return bad_intputs;
  }
  ...
  if (ml == 0) {
    ...
    streamlog_out(ERROR) << ">>>>>>>>>>>  no measurment layer found for trkhit cellid0 : "
                         << cellIDString(trkhit->getCellID0()) << " at " << Vector3D(trkhit->getPosition())
                         << std::endl;
  ...
  if (error_code != success) {
    ...
    streamlog_out(DEBUG2) << ">>>>>>>>>>>  addAndFit Number of Outliers : " << _outlier_chi2_values.size() << std::endl;
  ...
  if (_trackHitAtPositiveNDF == 0 && _kaltrack->GetNDF() >= 0) {
    ...
    streamlog_out(DEBUG2) << ">>>>>>>>>>>  Fit is now constrained at : " << cellIDString(trkhit->getCellID0())
                          << " pos " << Vector3D(trkhit->getPosition()) << " trkhit = " << _trackHitAtPositiveNDF
                          << " index of kalhit = " << _hitIndexAtPositiveNDF << " NDF = " << _kaltrack->GetNDF()
                          << std::endl;
  }
...
int MarlinDDKalTestTrack::testChi2Increment(EVENT::TrackerHit* trkhit, double& chi2increment) {
  ...
  int error_code = this->addAndFit(kalhit, chi2increment, site,
                                   -DBL_MAX); // using -DBL_MAX here ensures the hit will never be added to the fit
...
int MarlinDDKalTestTrack::getTrackState(EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts, double& chi2, int& ndf) {
  ...
  streamlog_out(DEBUG2)
      << "MarlinDDKalTestTrack::getTrackState( EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts ) using hit: "
      << trkhit << " with cellID0 = " << trkhit->getCellID0() << std::endl;
...
int MarlinDDKalTestTrack::extrapolate(const Vector3D& point, EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts,
                                      double& chi2, int& ndf) {
...
int MarlinDDKalTestTrack::extrapolate(const Vector3D& point, const TKalTrackSite& site, IMPL::TrackStateImpl& ts,
                                      double& chi2, int& ndf) {

  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::extrapolate( const Vector3D& point, IMPL::TrackStateImpl& ts, "
                           "double& chi2, int& ndf ) called "
                        << std::endl;
...
int MarlinDDKalTestTrack::extrapolateToLayer(int layerID, IMPL::TrackStateImpl& ts, double& chi2, int& ndf,
...
int MarlinDDKalTestTrack::extrapolateToLayer(int layerID, EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts,
...
int MarlinDDKalTestTrack::extrapolateToLayer(int layerID, const TKalTrackSite& site, IMPL::TrackStateImpl& ts,
                                             double& chi2, int& ndf, int& detElementID, int mode) {

  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::extrapolateToLayer( int layerID, IMPL::TrackStateImpl& ts, double& "
                           "chi2, int& ndf, int& detElementID ) called "
                        << std::endl;
...
int MarlinDDKalTestTrack::extrapolateToDetElement(int detElementID, IMPL::TrackStateImpl& ts, double& chi2, int& ndf,
                                                  int mode) {
...
int MarlinDDKalTestTrack::extrapolateToDetElement(int detElementID, EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts,
                                                  double& chi2, int& ndf, int mode) {
...
int MarlinDDKalTestTrack::extrapolateToDetElement(int detElementID, const TKalTrackSite& site, IMPL::TrackStateImpl& ts,
                                                  double& chi2, int& ndf, int mode) {

  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::extrapolateToDetElement( int detElementID, IMPL::TrackStateImpl& ts, "
                           "double& chi2, int& ndf, int mode ) called "
                        << std::endl;
...
int MarlinDDKalTestTrack::propagate(const Vector3D& point, IMPL::TrackStateImpl& ts, double& chi2, int& ndf) {
  ...
  const DDVMeasLayer* ml =
      ((_ktest->getIPLayer()) && point.r() < _ktest->getIPLayer()->GetR()) ? _ktest->getIPLayer() : 0;

  return this->propagate(point, site, ts, chi2, ndf, ml);
}

int MarlinDDKalTestTrack::propagate(const Vector3D& point, EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts,
                                    double& chi2, int& ndf) {
  ...
  if (ml)
    if (point.r() > _ktest->getIPLayer()->GetR())
      ml = NULL;
...
int MarlinDDKalTestTrack::propagate(const Vector3D& point, const TKalTrackSite& site, IMPL::TrackStateImpl& ts,
                                    double& chi2, int& ndf, const DDVMeasLayer* ml) {

  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::propagate( const Vector3D& point, const TKalTrackSite& site, "
...
int MarlinDDKalTestTrack::propagateToLayer(int layerID, IMPL::TrackStateImpl& ts, double& chi2, int& ndf,
                                           int& detElementID, int mode) {
...
int MarlinDDKalTestTrack::propagateToLayer(int layerID, EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts,
                                           double& chi2, int& ndf, int& detElementID, int mode) {
...
int MarlinDDKalTestTrack::propagateToLayer(int layerID, const TKalTrackSite& site, IMPL::TrackStateImpl& ts,
                                           double& chi2, int& ndf, int& detElementID, int mode) {

  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::propagateToLayer( int layerID, const TKalTrackSite& site, "
...
int MarlinDDKalTestTrack::propagateToDetElement(int detElementID, IMPL::TrackStateImpl& ts, double& chi2, int& ndf,
                                                int mode) {
...
int MarlinDDKalTestTrack::propagateToDetElement(int detElementID, EVENT::TrackerHit* trkhit, IMPL::TrackStateImpl& ts,
                                                double& chi2, int& ndf, int mode) {
...
int MarlinDDKalTestTrack::propagateToDetElement(int detElementID, const TKalTrackSite& site, IMPL::TrackStateImpl& ts,
                                                double& chi2, int& ndf, int mode) {

  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::propagateToDetElement( int detElementID, IMPL::TrackStateImpl& ts, "
                           "double& chi2, int& ndf, int mode ) called "
                        << std::endl;
...
int MarlinDDKalTestTrack::intersectionWithDetElement(int detElementID, EVENT::TrackerHit* trkhit, Vector3D& point,
                                                     int mode) {
...
int MarlinDDKalTestTrack::intersectionWithDetElement(int detElementID, const TKalTrackSite& site, Vector3D& point,
                                                     const DDVMeasLayer*& ml, int mode) {

  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::intersectionWithDetElement( int detElementID, const TKalTrackSite& "
                           "site, Vector3D& point, const DDVMeasLayer*& ml, int mode) called "
                        << std::endl;
  ...
  int dummy_detElementID; // not returned here as this is a single element as far as the outside world is concerned.
                          // Could check they are equal if we wanted ...
  ...
  if (error_code == success) {
    ...
    streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::intersectionWithDetElement intersection with detElementID = "
                          << cellIDString(detElementID) << ": at x = " << point.x() << " y = " << point.y()
                          << " z = " << point.z() << std::endl;
  }
...
int MarlinDDKalTestTrack::intersectionWithLayer(int layerID, EVENT::TrackerHit* trkhit, Vector3D& point,
                                                int& detElementID, int mode) {
...
int MarlinDDKalTestTrack::intersectionWithLayer(int layerID, const TKalTrackSite& site, Vector3D& point,
                                                int& detElementID, const DDVMeasLayer*& ml, int mode) {

  streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::intersectionWithLayer( int layerID, const TKalTrackSite& site, "
  ...
  if (meas_modules.size() == 0) {
    ...
    streamlog_out(DEBUG5) << "MarlinDDKalTestTrack::intersectionWithLayer layer id unknown: layerID = "
                          << cellIDString(layerID) << std::endl;
  ...
  if (error_code == success) {
    ...
    streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::intersectionWithLayer intersection with layerID = " << layerID
                          << ": at x = " << point.x() << " y = " << point.y() << " z = " << point.z()
                          << " r = " << point.rho() << " detElementID = " << detElementID << " "
                          << cellIDString(detElementID) << std::endl;

  } else if (error_code == no_intersection) {

    ml = 0;
    streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::intersectionWithLayer No intersection with layerID = " << layerID
                          << " " << cellIDString(layerID) << std::endl;
  }
...
int MarlinDDKalTestTrack::findIntersection(const DDVMeasLayer& meas_module, const TKalTrackSite& site, Vector3D& point,
                                           double& dphi, int& detElementID, int mode) {
  ...
  //   streamlog_out(DEBUG2) << "************ MarlinDDKalTestTrack::intersectionWithLayer : using smoothed TrackState
  //   !!!!! " << std::endl ;
  ...
  //   streamlog_out(DEBUG2) << "************ MarlinDDKalTestTrack::intersectionWithLayer : using filtered TrackState
  //   !!!!! " << std::endl ;
  // //------------------------------
  ...
  streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::intersectionWithLayer crossing_exist = " << crossing_exist << " dphi "
...
int MarlinDDKalTestTrack::findIntersection(std::vector<DDVMeasLayer const*>& meas_modules, const TKalTrackSite& site,
                                           Vector3D& point, int& detElementID, const DDVMeasLayer*& ml, int mode) {
...
void MarlinDDKalTestTrack::ToLCIOTrackState(const THelicalTrack& helix, const TMatrixD& cov, IMPL::TrackStateImpl& ts,
  ...
  streamlog_out(DEBUG2) << " kaltest track parameters: "
                        ...
                        << " - r: " << std::sqrt(pivot[0] * pivot[0] + pivot[1] * pivot[1]) << "]" << std::endl;
}

void MarlinDDKalTestTrack::ToLCIOTrackState(const TKalTrackSite& site, IMPL::TrackStateImpl& ts, double& chi2,
                                            int& ndf) const {
...
int MarlinDDKalTestTrack::getSiteFromLCIOHit(EVENT::TrackerHit* trkhit, TKalTrackSite*& site) const {
  ...
  if (it == _hit_used_for_sites.end()) { // hit not associated with any site
    ...
    for (unsigned int i = 0; i < _hit_not_used_for_sites.size(); ++i) {
      if (trkhit == _hit_not_used_for_sites[i])
        found = true;
    }

    if (found) {
      streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::getSiteFromLCIOHit: hit was rejected during filtering"
                            << std::endl;
      return site_discarded;
    } else {
      streamlog_out(DEBUG2) << "MarlinDDKalTestTrack::getSiteFromLCIOHit: hit " << trkhit
                            << " not in list of supplied hits" << std::endl;
      return bad_intputs;
    }
  }
  ...
  streamlog_out(DEBUG1) << "MarlinDDKalTestTrack::getSiteFromLCIOHit: site " << site << " found for hit " << trkhit
                        << std::endl;
  return success;
}
