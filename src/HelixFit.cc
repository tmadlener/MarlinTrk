...
int HelixFit::fastHelixFit(int npt, double* xf, double* yf, float* rf, float* pf, double* wf, float* zf, float* wzf,
                           int iopt, float* vv0, float* ee0, float& ch2ph, float& ch2z) {
...
#define MAX_CHI2 5000.0
  ...
  float sp2[MPT], del[MPT], deln[MPT], delzn[MPT], sxy[MPT], ss0[MPT], eee[MPT], delz[MPT], grad[5], cov[15], vv1[5],
      dv[5];

  //  double xf[MPT],yf[MPT],wf[MPT],zf[MPT],wzf[MPT];

  double alf, a0, a1, a2, a22, bet, cur, dd, den, det, dy, d2, f, fact, fg, f1, g, gam, gam0, g1, h, h2, p2, q2, rm, rn,
      xa, xb, xd, xi, xm, xx, xy, x1, x2, den2, ya, yb, yd, yi, ym, yy, y1, y2, wn, sa2b2, dd0, phic, aaa;
  ...
  for (int i = 0; i < npt; ++i) {
    ...
    //                std::cout << "ph0 + (ss0[i]-aa0)* M_PI_2 = " <<  ph0 + (ss0[i]-aa0)* M_PI_2  << " i = " << i <<
    //                std::endl; std::cout << "ss0[i]*asin(ff0) = " <<  ss0[i]*asin(ff0)  << " i = " << i <<  std::endl;
    //                std::cout << "ph0 + (ss0[i]-aa0)* M_PI_2 + ss0[i]*asin(ff0) = " << ph0 + (ss0[i]-aa0)* M_PI_2 +
    //                ss0[i]*asin(ff0) << std::endl; std::cout << "del[i] = " <<  del[i] << " i = " << i <<  std::endl;
  ...
  for (int i = 0; i < npt; ++i) {
    ...
    if (fabs(check) > 1.0 - eps)
      check = 2. * 0.007;

    double dzd =
        2.0 * (vv0[1] / vv0[0]) * fabs(ggg) * (0.5 * aa0 * vv0[0] / (1.0 - aa0 * vv0[3] * vv0[0]) - vv0[3] / check);

    double dzo = -vv0[1] * sxy[i] / vv0[0] +
                 vv0[1] * ggg / (vv0[0] * vv0[0]) * (2.0 + aa0 * vv0[0] * vv0[3] / (1.0 - aa0 * vv0[0] * vv0[3]));
  ...
  if (iopt < 3) {
    ...
    streamlog_out(DEBUG1) << "HelixFit: "
                          << " d0 = " << vv0[3] << " phi0 = " << vv0[2] << " omega = " << vv0[0] << " z0 = " << vv0[4]
                          << " tanL = " << vv0[1] << std::endl;
  ...
  if (error != 0) {
    streamlog_out(ERROR) << "CLHEP Matrix inversion failed"
                         << "return 1 " << std::endl;
    ch2ph = 1.0e30;
    ch2z = 1.0e30;
    return 1;
