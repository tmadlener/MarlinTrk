...
int createTrackStateAtCaloFace(IMarlinTrack* marlinTrk, IMPL::TrackStateImpl* track, EVENT::TrackerHit* trkhit,
                               bool tanL_is_positive);

int createFinalisedLCIOTrack(IMarlinTrack* marlinTrk, std::vector<EVENT::TrackerHit*>& hit_list, IMPL::TrackImpl* track,
                             bool fit_direction, const EVENT::FloatVec& initial_cov_for_prefit, float bfield_z,
  ...
  if (return_error == 0) {
    ...
    streamlog_out(DEBUG3) << "MarlinTrk::createFinalisedLCIOTrack : Prefit failed error = " << return_error
                          << std::endl;
  }
...
int createFinalisedLCIOTrack(IMarlinTrack* marlinTrk, std::vector<EVENT::TrackerHit*>& hit_list, IMPL::TrackImpl* track,
                             bool fit_direction, EVENT::TrackState* pre_fit, float bfield_z, double maxChi2Increment) {
...
int createFit(std::vector<EVENT::TrackerHit*>& hit_list, IMarlinTrack* marlinTrk, EVENT::TrackState* pre_fit,
              float bfield_z, bool fit_direction, double maxChi2Increment) {
  ...
  streamlog_out(DEBUG2) << "MarlinTrk::createFit Start Fit: AddHits: number of hits to fit " << hit_list.size()
                        << std::endl;
  ...
  if (ndof_added < MIN_NDF) {
    streamlog_out(DEBUG2) << "MarlinTrk::createFit : Cannot fit less with less than " << MIN_NDF
  ...
  if (return_error != IMarlinTrack::success) {
    ...
    streamlog_out(DEBUG5) << "MarlinTrk::createFit Initialisation of track fit failed with error : " << return_error
                          << std::endl;
...
int createPrefit(std::vector<EVENT::TrackerHit*>& hit_list, IMPL::TrackStateImpl* pre_fit, float bfield_z,
                 bool fit_direction) {
  ...
  if (twoD_hits.size() < 3) { // no chance to initialise print warning and return
    streamlog_out(WARNING) << "MarlinTrk::createFinalisedLCIOTrack Cannot create helix from less than 3 2-D hits"
                           << std::endl;
    return IMarlinTrack::bad_intputs;
  }
  ...
  //   helixTrack.moveRefPoint(hit_list.front()->getPosition()[0], hit_list.front()->getPosition()[1],
...
int finaliseLCIOTrack(IMarlinTrack* marlintrk, IMPL::TrackImpl* track, std::vector<EVENT::TrackerHit*>& hit_list,
                      bool fit_direction, IMPL::TrackStateImpl* atLastHit, IMPL::TrackStateImpl* atCaloFace) {
  ...
  EVENT::TrackerHit* firstHit =
      (fit_direction == IMarlinTrack::backward ? hits_in_fit.back().first : hits_in_fit.front().first);

  ///////////////////////////////////////////////////////
  // last hit
  ///////////////////////////////////////////////////////

  EVENT::TrackerHit* lastHit =
      (fit_direction == IMarlinTrack::backward ? hits_in_fit.front().first : hits_in_fit.back().first);
  ...
  streamlog_out(DEBUG4) << "MarlinTrk::finaliseLCIOTrack: finalised kaltest track  : " << marlintrk->toString()
                        << std::endl;
  ...
  if (fit_direction == IMarlinTrack::backward || usingAidaTT) {
    ...
    double _bfield = 42.0;
    // fixme: the implementation for DDKalTest does no longer need this value but the IMarlinTrk interface is not yet
    // changed
    ...
    while (hI != hits_in_fit.rend()) {
      ...
      streamlog_out(DEBUG3) << " MarlinTrk::finaliseLCIOTrack: hit " << toString(h)
                            << "  added : " << MarlinTrk::errorCode(addHit) << "  deltaChi2: " << deltaChi << std::endl;

      if (addHit != MarlinTrk::IMarlinTrack::success) {

        streamlog_out(ERROR) << " ****  MarlinTrk::finaliseLCIOTrack:  could not add inner hit to track !!! "
                             << std::endl;
      }
    ...
    streamlog_out(DEBUG4) << " ***  MarlinTrk::finaliseLCIOTrack: - propagated temporary track fromfirst hit to IP : "
                          << toString(trkStateIP) << std::endl;
  }

  if (return_error != IMarlinTrack::success) {
    streamlog_out(DEBUG4) << "MarlinTrk::finaliseLCIOTrack: return_code for propagation = " << return_error
                          << " NDF = " << ndf << std::endl;
    delete trkStateIP;
    delete trkStateAtFirstHit;
    //      delete trkStateAtLastHit;
  ...
  if (return_error == IMarlinTrack::success) {
    ...
    streamlog_out(WARNING) << "  >>>>>>>>>>>MarlinTrk::finaliseLCIOTrack:  MarlinTrk::finaliseLCIOTrack:  could not "
  ...
  double r_first =
      firstHit->getPosition()[0] * firstHit->getPosition()[0] + firstHit->getPosition()[1] * firstHit->getPosition()[1];
  ...
  if (atLastHit == 0 && atCaloFace == 0) {
    ...
    streamlog_out(DEBUG5) << "  >>>>>>>>>>> MarlinTrk::finaliseLCIOTrack: create TrackState AtLastHit : using trkhit "
                          << last_constrained_hit << std::endl;
    ...
    if (return_error == IMarlinTrack::success) {
      ...
      streamlog_out(DEBUG5) << "  >>>>>>>>>>> MarlinTrk::finaliseLCIOTrack:  could not get TrackState at Last Hit "
                            << last_constrained_hit << std::endl;
      delete trkStateAtLastHit;
    }
    ...
    if (return_error == IMarlinTrack::success) {
      ...
      streamlog_out(DEBUG9) << "  >>>>>>>>>>> MarlinTrk::finaliseLCIOTrack:  could not get TrackState at Calo Face "
                            << std::endl;
      delete trkStateCalo;
...
int createTrackStateAtCaloFace(IMarlinTrack* marlintrk, IMPL::TrackStateImpl* trkStateCalo, EVENT::TrackerHit* trkhit,
                               bool tanL_is_positive) {

  streamlog_out(DEBUG5) << "  >>>>>>>>>>> createTrackStateAtCaloFace : using trkhit " << UTIL::toString(trkhit)
                        << " tanL_is_positive = " << tanL_is_positive << std::endl;
  ...
  return_error_barrel = marlintrk->propagateToLayer(encoder.lowWord(), trkhit, tsBarrel, chi2, ndf, detElementID,
  ...
  return_error_endcap = marlintrk->propagateToLayer(encoder.lowWord(), trkhit, tsEndcap, chi2, ndf, detElementID,
                                                    IMarlinTrack::modeForward);
  ...
  streamlog_out(DEBUG2)
      << "  >>>>>>>>>>> createTrackStateAtCaloFace : setting d0 and z0 to 0. for track state at calorimeter : "
...
void addHitNumbersToTrack(IMPL::TrackImpl* track, std::vector<EVENT::TrackerHit*>& hit_list, bool hits_in_fit,
...
void addHitNumbersToTrack(IMPL::TrackImpl* track, std::vector<std::pair<EVENT::TrackerHit*, double>>& hit_list,
