...
int createFinalisedLCIOTrack(IMarlinTrack* marlinTrk, std::vector<EVENT::TrackerHit*>& hit_list, IMPL::TrackImpl* track,
                             bool fit_direction, EVENT::TrackState* pre_fit, float bfield_z,
...
/** Takes a list of hits and uses the IMarlinTrack inferface to fit them using a supplied prefit containing a covariance
 * matrix for the initialisation. */
int createFit(std::vector<EVENT::TrackerHit*>& hit_list, IMarlinTrack* marlinTrk, EVENT::TrackState* pre_fit,
              float bfield_z, bool fit_direction, double maxChi2Increment = DBL_MAX);
...
int finaliseLCIOTrack(IMarlinTrack* marlinTrk, IMPL::TrackImpl* track, std::vector<EVENT::TrackerHit*>& hit_list,
                      bool fit_direction, IMPL::TrackStateImpl* atLastHit = 0, IMPL::TrackStateImpl* atCaloFace = 0);
